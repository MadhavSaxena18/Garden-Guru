...
class SignupViewController: UIViewController {
    ...
    @objc private func signupButtonTapped() {
        ...
        case .initial:
            ...
            showLoadingIndicator()
            viewModel.email = email
            viewModel.password = password // Store password in view model
            Task {
                await viewModel.checkEmailAndSendOTP()
            }
        ...
        case .otp:
            ...
            // Validate OTP format (should be numeric and 6 digits)
            guard otp.count == 6, otp.allSatisfy({ $0.isNumber }) else {
                showAlert(title: "Error", message: "Please enter a valid 6-digit verification code")
                return
            }
